...
results_dir = os.path.join(script_dir, 'chanceheatmaps/')
sample_file_name = filename

csv_filename = 'chanceheatmaps/'+sample_file_name+'.csv'
...
while (times_count > 0) and (index < 8):
    boundaries_list.append(index * filter)
    index = index + 1
    times_count -= 1
...
while times_count > 0:
    print("index",index)
    boundaries_list.append(index * (filter - filter * 2))
    index = index + 1
    times_count -= 1
